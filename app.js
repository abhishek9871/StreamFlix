...
class CinematicTV {
    ...
    createContentCard(item, type) {
        ...
        const imageUrl = item.poster_path ? `${this.imageBaseURL}${item.poster_path}` : null;
    ...
    navigateUp(element) {
        const parent = element.closest('.content-rail') || element.closest('.section-header');
    ...
    handleBack() {
        if (document.querySelector('.modal.active') || document.querySelector('.video-modal.active')) {
            this.closeModal();
            return;
        }
    ...
    async loadFilteredContent(filter) {
        ...
        switch(section) {
            ...
            case 'tv':
                ...
                else if (filter === 'top_rated') items = await this.getTopRatedTVShows();
    ...
    displayDetailsModal(details) {
        ...
        const posterUrl = details.poster_path ? `${this.imageBaseURL}${details.poster_path}` : null;
        const backdropUrl = details.backdrop_path ? `${this.imageBaseURL}${details.backdrop_path}` : null;
    ...
    async playNextEpisode(show, currentEpisode) {
        try {
            ...
            const currentSeason = seasons.find(s => s.season_number === currentEpisode.season);
            
            if (currentSeason && currentEpisode.episode < currentSeason.episode_count) {
                const nextEpisode = {
    ...
    setupEventListeners() {
        ...
        document.getElementById('videoClose').addEventListener('click', () => this.closeModal());
